...
impl ServiceType for JmixEndpoint {
    ...
    fn build_router(&self, options: &HashMap<String, Value>) -> Vec<RouteConfig> {
        ...
        let mut routes: Vec<RouteConfig> = vec![
            ...
            RouteConfig { path: format!("{}/api/jmix/{{id}}", base),           methods: vec![Method::GET],  description: Some("JMIX get envelope by id".to_string()) },
            // 1b. GET manifest by ID
            RouteConfig { path: format!("{}/api/jmix/{{id}}/manifest", base),  methods: vec![Method::GET],  description: Some("JMIX get manifest by id".to_string()) },
            // 1c. GET by query (e.g., studyInstanceUid)
            RouteConfig { path: format!("{}/api/jmix", base),                  methods: vec![Method::GET],  description: Some("JMIX query envelope".to_string()) },
            // 2. POST Envelope upload
            RouteConfig { path: format!("{}/api/jmix", base),                  methods: vec![Method::POST], description: Some("JMIX upload envelope".to_string()) },
        ];
...
fn resolve_store_root(options: &HashMap<String, Value>) -> PathBuf {
    ...
    // Default to ./tmp/jmix-store per user rule preference
...
fn make_targz(dir: &Path) -> Result<Vec<u8>, Error> {
    let mut buf = Vec::new();
    {
        ...
        tar.append_dir_all(".", dir).map_err(|e| Error::from(format!("tar error: {}", e)))?;
        tar.finish().map_err(|e| Error::from(format!("tar finish error: {}", e)))?;
...
fn make_zip(dir: &Path) -> Result<Vec<u8>, Error> {
    let mut buf = Vec::new();
    {
        ...
        let options = ZipFileOptions::default().compression_method(zip::CompressionMethod::Deflated);
        let base = dir;
        for entry in WalkDir::new(dir).into_iter().filter_map(|e| e.ok()) {
            ...
            if entry.file_type().is_dir() {
                if !name.is_empty() {
                    zip.add_directory(name.to_string(), options).map_err(|e| Error::from(format!("zip dir error: {}", e)))?;
                }
            } else if entry.file_type().is_file() {
                zip.start_file(name.to_string(), options).map_err(|e| Error::from(format!("zip file error: {}", e)))?;
                let data = fs::read(path).map_err(|e| Error::from(format!("zip read error: {}", e)))?;
                zip.write_all(&data).map_err(|e| Error::from(format!("zip write error: {}", e)))?;
            }
        }
        zip.finish().map_err(|e| Error::from(format!("zip finish error: {}", e)))?;
    }
    Ok(buf)
}
...
fn extract_zip(bytes: &[u8], dest: &Path) -> Result<(), Error> {
    ...
    let mut zip = zip::ZipArchive::new(reader).map_err(|e| Error::from(format!("zip open error: {}", e)))?;
    for i in 0..zip.len() {
        let mut file = zip.by_index(i).map_err(|e| Error::from(format!("zip idx error: {}", e)))?;
        let outpath = dest.join(file.mangled_name());
        if file.name().ends_with('/') {
            fs::create_dir_all(&outpath).map_err(|e| Error::from(format!("mkdir error: {}", e)))?;
        } else {
            if let Some(parent) = outpath.parent() { fs::create_dir_all(parent).map_err(|e| Error::from(format!("mkparent error: {}", e)))?; }
            let mut outfile = fs::File::create(&outpath).map_err(|e| Error::from(format!("create error: {}", e)))?;
            std::io::copy(&mut file, &mut outfile).map_err(|e| Error::from(format!("write error: {}", e)))?;
        }
    }
    Ok(())
}
...
fn extract_targz(bytes: &[u8], dest: &Path) -> Result<(), Error> {
    ...
    ar.unpack(dest).map_err(|e| Error::from(format!("targz unpack error: {}", e)))
}

fn find_package_root_and_manifest(extracted_root: &Path) -> Result<(PathBuf, serde_json::Value), Error> {
    // Look for manifest.json; choose dir containing it as package root
    for entry in WalkDir::new(extracted_root).into_iter().filter_map(|e| e.ok()) {
        if entry.file_type().is_file() && entry.file_name() == "manifest.json" {
            let pkg_dir = entry.path().parent().unwrap().to_path_buf();
            let s = fs::read_to_string(entry.path()).map_err(|e| Error::from(format!("manifest read error: {}", e)))?;
            let json: serde_json::Value = serde_json::from_str(&s).map_err(|e| Error::from(format!("manifest parse error: {}", e)))?;
...
fn query_by_study_uid(store_root: &Path, study_uid: &str) -> Result<Vec<serde_json::Value>, Error> {
    ...
    if !store_root.exists() { return Ok(results); }
    for entry in fs::read_dir(store_root).map_err(|e| Error::from(format!("readdir error: {}", e)))? {
...
impl ServiceHandler<Value> for JmixEndpoint {
    ...
    async fn transform_request(
        ...
        let mut set_response = |status: http::StatusCode, hdrs: HashMap<String, String>, body_str: Option<String>, json_obj: Option<serde_json::Value>, body_b64: Option<String>| {
            let mut resp = serde_json::Map::new();
            resp.insert("status".to_string(), serde_json::json!(status.as_u16()));
            if !hdrs.is_empty() {
                resp.insert("headers".to_string(), serde_json::json!(hdrs));
            }
            if let Some(s) = body_str { resp.insert("body".to_string(), serde_json::json!(s)); }
            if let Some(j) = json_obj { resp.insert("json".to_string(), j); }
            if let Some(b) = body_b64 { resp.insert("body_b64".to_string(), serde_json::json!(b)); }
        ...
        if method == "GET" && subpath.starts_with("api/jmix/") && !subpath.ends_with("/manifest") {
            ...
            if !rest.is_empty() && !rest.contains('/') {
                let id = rest;
                // Negotiate Accept
                let wants_gzip = accept.map(|a| a.contains("application/gzip") || a.contains("application/x-gtar")).unwrap_or(false);
                let wants_zip = accept.map(|a| a.contains("application/zip")).unwrap_or(false);
                let negotiated = if accept.is_none() || wants_zip || (!wants_gzip && accept == Some("*/*")) {
                ...
                match negotiated {
                    Some(ct) => {
                        ...
                        let filename = if ct == "application/gzip" { format!("{}{}.tar.gz", "", id) } else { format!("{}{}.zip", "", id) };
                        hdrs.insert("content-disposition".to_string(), format!("attachment; filename=\"{}\"", filename));

                        // Locate envelope directory
                        let package_dir = package_dir_for(&store_root, id);
                        if !package_dir.exists() {
                            set_response(http::StatusCode::NOT_FOUND, HashMap::new(), Some("Envelope not found".into()), None, None);
                            return Ok(envelope);
                        }
                        ...
                        let bytes = if ct == "application/gzip" {
                            match make_targz(&package_dir) {
                                Ok(b) => b,
                                Err(e) => {
                                    set_response(http::StatusCode::INTERNAL_SERVER_ERROR, HashMap::new(), Some(format!("archive error: {}", e)), None, None);
                                    return Ok(envelope);
                                }
                            }
                        } else {
                    ...
                    None => {
                        set_response(http::StatusCode::NOT_ACCEPTABLE, HashMap::new(), Some(String::from("unsupported media type in Accept")), None, None);
                    }
                }
                return Ok(envelope);
            }
            ...
            if let Some(id_part) = rest.strip_suffix("/manifest") {
                ...
                if !id.is_empty() && !id.contains('/') {
                    ...
                    if !manifest_path.exists() {
                        set_response(http::StatusCode::NOT_FOUND, HashMap::new(), Some("manifest.json not found".into()), None, None);
                        return Ok(envelope);
                    }
                    match fs::read_to_string(&manifest_path).ok().and_then(|s| serde_json::from_str::<serde_json::Value>(&s).ok()) {
                        Some(json) => {
                            set_response(http::StatusCode::OK, hdrs, None, Some(json), None);
                        }
                        None => {
                            set_response(http::StatusCode::INTERNAL_SERVER_ERROR, HashMap::new(), Some("failed to parse manifest.json".into()), None, None);
                        }
                    }
                    return Ok(envelope);
                }
        ...
        if method == "GET" && subpath == "api/jmix" {
            ...
            if let Some(uid) = study_uid {
                ...
                set_response(http::StatusCode::BAD_REQUEST, HashMap::new(), Some("missing studyInstanceUid".into()), None, None);
                return Ok(envelope);
            }
        }
        ...
        if method == "POST" && subpath == "api/jmix" {
            ...
            // Write body to temp dir under ./tmp as per rule
            ...
            if let Err(e) = extract_res {
                set_response(http::StatusCode::UNSUPPORTED_MEDIA_TYPE, HashMap::new(), Some(format!("extract error: {}", e)), None, None);
                return Ok(envelope);
            }

            // Find manifest.json in extracted content
            let (pkg_dir, manifest_json) = find_package_root_and_manifest(&extracted_root)?;
            let id = manifest_json.get("id").and_then(|v| v.as_str()).unwrap_or("");
            if id.is_empty() {
                set_response(http::StatusCode::BAD_REQUEST, HashMap::new(), Some("manifest.id missing".into()), None, None);
                return Ok(envelope);
            }
            ...
            if dest_dir.exists() {
                set_response(http::StatusCode::CONFLICT, HashMap::new(), Some("envelope id already exists".into()), None, None);
                return Ok(envelope);
            }
            fs::create_dir_all(&store_root).map_err(|e| Error::from(format!("store dir error: {}", e)))?;
            fs_extra::dir::copy(&pkg_dir, &store_root, &fs_extra::dir::CopyOptions { copy_inside: true, content_only: false, overwrite: false, ..Default::default() })
            ...
            let opts = jmix_rs::ValidationOptions { schema_dir: None, validate_schema: true, verify_assertions: false, recipient_secret_key_path: None };
            match jmix_rs::validate_package(&dest_dir, &opts) {
                ...
                Err(e) => {
                    ...
                    set_response(http::StatusCode::BAD_REQUEST, HashMap::new(), Some(format!("validation failed: {}", e)), None, None);
                }
            }
            return Ok(envelope);
        }
        ...
        set_response(http::StatusCode::NOT_FOUND, HashMap::new(), Some(String::from("Not Found")), None, None);
        Ok(envelope)
    }
    ...
    async fn transform_response(
        ...
        if let Some(body_b64) = response_meta.and_then(|m| m.get("body_b64")).and_then(|b| b.as_str()) {
        ...
        if let Some(json_val) = response_meta.and_then(|m| m.get("json")) {
            let body_str = serde_json::to_string(json_val).map_err(|_| Error::from("Failed to serialize JMIX response JSON"))?;
            if !has_content_type {
