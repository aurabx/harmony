...
async fn test_management_service_enabled() {
    ...
    let endpoint = config.endpoints.get("management").expect("Management endpoint not created");
    assert_eq!(endpoint.service, "management");
    
    // Verify pipeline was created
    let pipeline = config.pipelines.get("management").expect("Management pipeline not created");
    assert_eq!(pipeline.endpoints, vec!["management"]);
    assert!(pipeline.middleware.is_empty());

    // Verify service is properly registered
    let service = config.services.get("management").expect("Management service not registered");
    ...
    let service = endpoint.resolve_service().expect("Failed to resolve management service");
    service.validate(endpoint_options).expect("Service validation failed");
