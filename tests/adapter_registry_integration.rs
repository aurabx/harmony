...
async fn test_registry_creation() {
    ...
    assert!(
        networks.is_empty(),
        "New registry should have no running networks"
    );
}
...
async fn test_registry_default() {
    ...
    assert!(
        networks.is_empty(),
        "Default registry should have no running networks"
    );
}
...
async fn test_start_network_success() {
    ...
    // Start network
    let result = registry
        .start_network("test_network".to_string(), config_arc.clone())
        .await;
...
async fn test_start_network_invalid() {
    ...
    let result = registry
        .start_network("nonexistent_network".to_string(), config_arc)
        .await;
    ...
    assert!(
        networks.is_empty(),
        "Should have no running networks after failure"
    );
}
...
async fn test_stop_network_success() {
    ...
    // Start network
    registry
        .start_network("stop_test".to_string(), config_arc)
        .await
        .ok();
    ...
    assert!(
        networks.is_empty(),
        "Should have no running networks after stop"
    );
}
...
async fn test_stop_network_nonexistent() {
    ...
    assert!(
        result.is_ok(),
        "Stopping non-existent network should be no-op"
    );
}
...
async fn test_restart_network_success() {
    ...
    // Start network
    registry
        .start_network("restart_test".to_string(), config_arc.clone())
        .await
        .ok();

    // Restart network
    let result = registry
        .restart_network("restart_test".to_string(), config_arc)
        .await;
...
async fn test_stop_all_with_multiple_networks() {
    ...
    registry
        .start_network("network1".to_string(), config_arc.clone())
        .await
        .ok();
    registry
        .start_network("network2".to_string(), config_arc.clone())
        .await
        .ok();
    registry
        .start_network("network3".to_string(), config_arc.clone())
        .await
        .ok();
    ...
    assert!(
        networks.is_empty(),
        "Should have no running networks after stop_all"
    );
}
...
async fn test_get_running_networks() {
    let temp_dir = TempDir::new().unwrap();
    let config_path =
        create_multi_network_config(&temp_dir, vec![("net_a", 18087), ("net_b", 18088)]);
    ...
    // Start first network
    registry
        .start_network("net_a".to_string(), config_arc.clone())
        .await
        .ok();
    ...
    registry
        .start_network("net_b".to_string(), config_arc.clone())
        .await
        .ok();
...
async fn test_concurrent_start_operations() {
    ...
    let handle1 = tokio::spawn(async move {
        registry1
            .start_network("concurrent1".to_string(), config1)
            .await
    });
    ...
    let handle2 = tokio::spawn(async move {
        registry2
            .start_network("concurrent2".to_string(), config2)
            .await
    });
...
async fn test_adapter_shutdown_timeout() {
    ...
    // Start network
    registry
        .start_network("timeout_test".to_string(), config_arc)
        .await
        .ok();
    ...
    let stop_result = timeout(
        ...
        registry.stop_network("timeout_test"),
    )
    .await;
...
async fn test_network_lifecycle_state_consistency() {
    ...
    // Start network
    registry
        .start_network("lifecycle_test".to_string(), config_arc.clone())
        .await
        .ok();
    assert_eq!(registry.get_running_networks().await.len(), 1);
    ...
    registry
        .restart_network("lifecycle_test".to_string(), config_arc)
        .await
        .ok();
    assert_eq!(registry.get_running_networks().await.len(), 1);
...
async fn test_repeated_start_same_network() {
    ...
    let result1 = registry
        .start_network("repeated_test".to_string(), config_arc.clone())
        .await;
    ...
    let _result2 = registry
        .start_network("repeated_test".to_string(), config_arc)
        .await;
