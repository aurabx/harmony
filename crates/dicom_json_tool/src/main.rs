...
#[derive(Parser, Debug)]
#[command(name = "dicom-json", about = "Convert between DICOM datasets and DICOM JSON")] 
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum Cmd {
    ...
    FromJson { #[arg(short, long)] input: PathBuf, #[arg(short, long)] output: PathBuf },
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::FromJson { input, output } => {
            ...
            let obj: InMemDicomObject = tool::json_value_to_identifier(&identifier)
